...
[{
  ...
  const saveToCookbook = async () => {
    if (!madeModal) return
    setSaving(true)
    const { data } = await sb.from("cookbook").insert({ user_id: user.id, name: madeModal.name, emoji: madeModal.emoji, description: madeModal.description, stars, notes, people }).select().single()
